...
import {providerSchema,capabilitySchema,validateSchema,validateValue,boundedJson,splitAmount,DELIVERY_TYPES} from './marketplace-schema.mjs';
...
export function createMarketplace({core,dataFile=path.resolve('data/marketplace.json'),feeBps=1000,adapters=createAdapters(),maxRecords=10000}={}) {
  ...
  function eligible(s,capabilityId) {
    ...
    if(!c||c.status!=='active'||s.providers[c.providerId]?.status!=='active'||c.visibility!=='public') throw new Error('CAPABILITY_DISABLED');
  ...
  async function registerCapability(input,principal) {
    boundedJson(input);const cap=capabilitySchema.parse(input);owner(principal,cap.providerId);
    ...
    return update(s=>{capacity(s.capabilities);if(s.providers[cap.providerId]?.status!=='active')throw new Error('PROVIDER_INACTIVE');
      ...
      const capability={...cap,capabilityId:uid('capability'),status:'planned',createdAt:now(),updatedAt:now(),
        ...
        rights:{...cap.rights,affirmationTimestamp:now(),affirmationVersion:'ck-rights/1',ownershipTransferred:false},
  ...
  return {dataFile,describe,registerProvider,applyCreator,registerCapability,setProviderStatus,setCapabilityStatus,getProvider,getCapability,search,quote,approvePurchase,purchase,jobStatus,getReceipt,listAllocations,getProviderBalance,markSettled,download};
}
