...
export function decodeTransfers(receipt, receiver) {
  ...
  for (const log of Array.isArray(receipt?.logs) ? receipt.logs : []) {
    ...
    if (!Array.isArray(log.topics) || address(log.topics[0]) !== TRANSFER_TOPIC) continue;
    try {
      if (log.topics.length !== 3 || typeof log.data !== 'string' || !/^0x[0-9a-fA-F]{64}$/.test(log.data)) throw new Error('Malformed Transfer log');
