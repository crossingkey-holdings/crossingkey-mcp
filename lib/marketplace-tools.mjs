...
export function registerMarketplaceTools(server,{marketplace,principal,legacyCapabilities,core,rate=()=>{}}) {
  ...
  add('capability.search',searchSchema,'FREE, read-only marketplace search. Use this for filtered discovery of active public marketplace capabilities by text, category, provider, delivery type, or price; use catalog.list for unfiltered browsing and capability.get for one exact identifier.',false,args=>marketplace.search(args));
  add('catalog.list',z.object(paging).strict(),'FREE, read-only marketplace browse. Use this to page through active public marketplace capabilities without search criteria; use capability.search for filtering and capability.get for exact metadata.',false,args=>marketplace.search(args));
  add('commerce.quote',z.object({capabilityId:id.describe('Exact active public marketplace capability identifier to quote.')}).strict(),'FREE, read-only marketplace purchase quote. Supply a marketplace capabilityId to receive exact gross price, CrossingKey fee, provider share, and x402 v1/v2 requirements; no payment occurs. For deterministic first-party machine capabilities use machine_capability.quote.',false,({capabilityId})=>marketplace.quote(capabilityId));
  add('capability.purchase',purchaseSchema,'PAID authenticated buyer action. Canonical marketplace purchase using a signed x402 payment plus a prior buyer-bound human approval. Call commerce.quote first. Payment is settled before bounded execution; failed delivery retains receipt/dispute accounting, and background tasks are forbidden.',true,(args,p)=>marketplace.purchase(args,p));
