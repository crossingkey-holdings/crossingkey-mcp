...
Other local commands: provider-status, capability-status, affirm-rights and mark-settled. Status requests contain providerId/capabilityId and status; capability-status may include visibility. affirm-rights contains capabilityId and the complete rights object, and may be run only after the operator actually makes that affirmation. Seed import does not make one for them. mark-settled contains allocationId and a reference to a separately approved completed transfer. It records accounting only. Administrator MCP equivalents enforce the same role checks. Settlement changes do not imply permission to move funds.
